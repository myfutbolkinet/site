...
class FuncCategoriesClass
{
    ...
    public function save_cats_filter(Request $request){
        ...
        $is_cat=\App\SiteGoodsFilter::where('user_id', $user)
            ->get();
        if(count($is_cat)>0){
            $is_cat = \App\SiteGoodsFilter::find($is_cat[0]->id);
           $is_cat->categories = $cats_array;
        }
        else{
            $is_cat = new \App\SiteGoodsFilter();
