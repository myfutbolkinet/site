...
// upload files to $storeFolder
    ...
    public function add_category_photo(Request $request){
        // define absolute folder path
        $storeFolder = storage_path().'/app/public/';
// if folder doesn't exists, create it
        if(!file_exists($storeFolder) && !is_dir($storeFolder)) {
...
// upload files to $storeFolder
        if (!empty($_FILES)) {
            ...
            if(move_uploaded_file($tempFile,$targetFile)) {
                ...
                $background = Image::canvas(230, 230);
                $background->fill('#fff');
                ...
                if($width>$height){
                    ...
                    $img->resize(230,230/$aspect_ratio );
                    $rest=(230-230/$aspect_ratio)/2;
                    $background->insert($img,'top-left', intval(0),intval($rest));
                }
                elseif($height>$width){

                    $aspect_ratio=$height/$width;
                    $img->resize(230/$aspect_ratio, 230);
                    $rest=(230-230/$aspect_ratio)/2;
                    $background->insert($img,'top-left', intval($rest),intval(0));
                }
                elseif($height==$width){
                ...
                $items = session('category_image');

                if ($items == null) {
                    session()->push('category_image', $target);
